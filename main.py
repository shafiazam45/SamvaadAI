...
with col1:
    st.header("Summary")
    st.markdown("Good job rehearsing! Keep up the hard work.")
    st.markdown("")
    st.markdown("")
    ...
    # st.markdown(f"Pace: {speech_rate_mean} syllables/min")
...
with col2:
    ...
    st.markdown(f"Average: {speech_rate_mean} syllables/min")
    st.markdown(f"Variation: ±{speech_rate_std} syllables/min")
    st.markdown("")
    st.markdown("")
    fig, ax = plt.subplots()
...
with col3:
    st.header("Pitch")
    # st.markdown(f"Mean: {pitch_mean} Hz")
    # st.markdown(f"Variation: ±{pitch_std} Hz")
    st.markdown(f"Low pitch variation will make your audience lose interest. Try increasing the tone\
             for your key points")
