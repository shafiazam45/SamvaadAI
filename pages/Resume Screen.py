...
def load_lottiefile(filepath: str):
    with open(filepath, "r") as f:
        return json.load(f)
st_lottie(load_lottiefile(r"C:\Users\Arpan Kumar\Downloads\GPTInterviewer-main (3)\GPTInterviewer-main\GPTInterviewer-main\images\welcome.json"), speed=1, reverse=False, loop=True, quality="high", height=300)

#st.markdown("""solutions to potential errors:""")

st.markdown("""\n""")
position = st.selectbox("Select the role", ["Software Engineer", "Data Scientist", "Product Manager"])
resume = st.file_uploader("Upload resume", type=["pdf"])
auto_play = st.checkbox("Enable speech mode")
